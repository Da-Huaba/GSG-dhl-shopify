...
// Erzeugt einen store-gebundenen Client (eigener Token-Cache je Store)
function createClient(store){
  ...
  async function* findCandidates(){
    ...
    let cursor=null;
    do{
      const d=await gql(`query($q:String!,$c:String){ orders(first:25, query:$q, after:$c, sortKey:CREATED_AT){
        ...
        nodes{ id name createdAt email tags
          ...
          returns(first:5){ nodes{ id name status
            ...
            reverseFulfillmentOrders(first:5){ nodes{ id reverseDeliveries(first:1){ nodes{ id } } lineItems(first:50){ nodes{ id totalQuantity } } } } } } } } }`, { q, c:cursor });
      for(const o of d.orders.nodes){
        ...
        for(const r of (o.returns?.nodes||[])){
          ...
          if(rfo && (rfo.reverseDeliveries?.nodes||[]).length>0) continue;
  ...
  async function getCustomsItems(returnGid){
    ...
    const d=await gql(`query($id:ID!){ return(id:$id){ returnLineItems(first:50){ nodes{ ... on ReturnLineItem {
      quantity fulfillmentLineItem{ lineItem{ title discountedUnitPriceSet{ shopMoney{ amount currencyCode } } } } } } } } }`, { id:returnGid });
    const nodes=d.return?.returnLineItems?.nodes||[];
    return nodes.map(n=>{ const li=n.fulfillmentLineItem?.lineItem||{}; const m=li.discountedUnitPriceSet?.shopMoney||{};
      return { itemDescription:(li.title||'Returned item').slice(0,50), packagedQuantity:n.quantity, itemWeight:{ uom:'kg', value:defKg }, itemValue:{ currency:m.currencyCode||'EUR', value:Number(m.amount||0) } }; });
  }
  async function approveReturn(id){
    const d=await gql(`mutation($in:ReturnApproveRequestInput!){ returnApproveRequest(input:$in){ return{ id status } userErrors{ message } } }`, { in:{ id, notifyCustomer:false } });
  ...
  async function createReverseDelivery(rfoId, lineItems, tracking, fileUrl, notify){
    const d=await gql(`mutation($rfoId:ID!,$li:[ReverseDeliveryLineItemInput!]!,$tr:ReverseDeliveryTrackingInput,$lb:ReverseDeliveryLabelInput,$n:Boolean){
        reverseDeliveryCreateWithShipping(reverseFulfillmentOrderId:$rfoId, reverseDeliveryLineItems:$li, trackingInput:$tr, labelInput:$lb, notifyCustomer:$n){ reverseDelivery{ id } userErrors{ message } } }`,
  ...
  return { key:store.key, shop:store.shop, findCandidates, getRfo, getCustomsItems, approveReturn, uploadLabel, createReverseDelivery, tagProcessed };
}
